# SỔ NHẬT KÝ NGHIÊN CỨU — GOLDEN WATCH (bản electronic gốc)
...
> Minh bạch: ngày có dấu vết máy tính (file, mtime, git, log) ghi theo bằng
> chứng thật; ngày KHÔNG còn dấu vết được tái dựng từ kế hoạch tuần tương ứng
> và đánh dấu *[r]* cuối entry — đội rà lại và bổ sung chi tiết cá nhân khi chép sổ.
...
*(Để trống 2–3 trang đầu sổ giấy, điền số trang sau khi hoàn thành.)*
...
2. **DỤNG CỤ & VẬT LIỆU** — Bài viết Hội Đột quỵ Việt Nam, truyền thông BV
   Chợ Rẫy/115.
3. **TIẾN TRÌNH THỰC HIỆN & HIỆN TƯỢNG**
   - Ghi: cửa sổ can thiệp tiêu chuẩn溶栓 4,5h; tỷ lệ người đến viện trong
     cửa sổ này ở VN còn thấp (cần số chính xác — ghi vào danh mục xin BV).
...
3. **TIẾN TRÌNH THỰC HIỆN & HIỆN TƯỢNG**
   - Xem xét CASIA (viển,trung Quốc, khỏe), PhysioNet "Gait in Aging and
...
3. **TIẾN TRÌNH THỰC HIỆN & HIỆN TƯỢNG**
   ...
   - Trích Phụ lục 1 (khai báo AI), Phụ lục 2 (sổ nhật ký), Phụ lục 3
     (poster) từ quy chế KHKT 2026–2027 lưu vào "Giaays tờ/".
...
4. **KẾ QUẢ & SỐ LIỆU THÔ** — Công thức v1; ngưỡng cảnh báo tạm 85%.
...
4. **KẾ QUẢ & SỐ LIỆU THÔ** — Chart chạy được; ngưỡng 85% hiển thị.
...
5. **RÚT KINH NGHIỆM & LỖI SAI**
   - Hệ chạy được ≠ hệ đúng: số 0.94 lúc đó là "tự ph Anh" — phải validate
...
3. **TIẾN TRÝNH THỰC HIỆN & HIỆN TƯỢNG**
...
4. **KẾT QUẢ & SỐ LIỆU THÔ** — Gait LOSO **0.879**; face rules **0.638**;
   M1-06 **d = 0.759**; face ML v2.5 **0.845**; metrics_pack 22:32.
...
3. **TIẾN TRÌNH THỰC HIỆN & HIỆN TƯỢNG**
   ...
   - **NK-03 LEAKAGE #2 (speech):** phát hiện split cũ là session×mic —
     ...
     LogReg **AUC 0.620** / MLP **0.609**; cross-mic head→array **0.415**
     (<0.5). Số 0.992 cũ = session-level. Output:
     `test_results/speech_speaker_loso_20260910_214834/`.
   - **NK-04:** face v3 đa seed 10 seeds → **AUC 0.9398 ± 0.0099
     [0.9282–0.9557]** → công bố dạng "0.94 ± 0.01 (10 seed)".
   - **NK-05:** NIHSS video toolkit `training/nihss_video_toolkit.py`
     ...
     (output smoke đã xóa).
   - **NK-06:** openSMILE eGeMAPSv02 LOSO → **AUC 0.663** (so LogReg 0.620).
...
3. **TIẾN TRÌNH THỰC HIỆN & HIỆN TƯỢNG**
   ...
   - **NK-10 Trụ cột A (abstention):** face uncertainty **0.777**; nhường
     20% mẫu khó nhất → FPR **9.5 → 5.3** giữ sens **91.1**; speech uncertainty
     **0.488** (thấp — ghi nhận trung thực).
   - **NK-11 Trụ cột B (cảnh báo sớm, simulated):** EWMA+CUSUM → FAR
     **0/24h**; giảm độ trễ **−45–50%**; ramp **0/20 vs 20/20 @21.6ph**.
   - **NK-12 Train 100% TORGO:** 17,631/17,633 file (2 lỗi F01_0067/0068),
     48 ft, 150 epochs, loss hội tụ ~0.13 → `speech_torgo_full_20260912_
     183425.pth` — **mô hình NGHIÊN CỨU**; production giữ model 28/08; số
     công bố vẫn là LOSO 0.620.
...
4. **KẾ QUẢ & SỐ LIỆU THÔ** — A: FPR 9.5→5.3, sens 91.1, face unc 0.777;
