...
class ArmWeaknessDetector:
    ...
    def detect_arm_weakness(self, frame):
        ...
        # Get keypoints — YOLO .xy trả (17,2); ghép thêm trục confidence
        # để được (17,3) đúng như phần còn lại của module kỳ vọng
        kp_xy = pose_results[0].keypoints.xy[0].cpu().numpy()
        kconf = pose_results[0].keypoints.conf
        if kconf is not None:
            conf = kconf[0].cpu().numpy()
        else:
            conf = np.ones(len(kp_xy), dtype=kp_xy.dtype)
        keypoints = np.column_stack([kp_xy, conf])  # (17, 3)
