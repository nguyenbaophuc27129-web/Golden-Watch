...
class SpeechAnalysisModule:
    ...
    def record_audio(self, duration_seconds=5):
        ...
        try:
            ...
            # AGC đơn giản — mic xa (camera gác cao) / nói nhỏ: âm quá nhỏ
            # (peak < 8%) được khuếch đại về mức dùng được, không méo dạng số
            peak = float(np.max(np.abs(audio_float))) if len(audio_float) else 0.0
            if 0.0 < peak < 0.08:
                audio_float = np.clip(audio_float * (0.30 / peak), -1.0, 1.0)
                print(f"[SPEECH] AGC: mic xa/nói nhỏ (peak {peak:.3f}) "
                      f"→ khuếch đại x{0.30/peak:.1f}")
