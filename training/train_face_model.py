...
class FaceStrokeDataset(Dataset):
    ...
    def __init__(self, data_frame, augment=False):
        ...
        # FIX M1-05b: mediapipe 1.0.x đã BỎ solutions API → dùng Tasks API
        # FaceLandmarker (IMAGE mode, cùng file .task với face_module_v7)
        task_file = os.path.join(current_dir_parent := os.path.dirname(
            os.path.dirname(os.path.abspath(__file__))),
            'models', 'face_landmarker_v2',
            'face_landmarker_v2_with_blendshapes.task')
        if not os.path.exists(task_file):
            task_file = os.path.join(current_dir_parent, 'src', 'models',
                                     'face_landmarker.task')
        from mediapipe.tasks.python import vision as mp_vision
        from mediapipe.tasks.python import BaseOptions as mp_BaseOptions
        base_opts = mp_BaseOptions(model_asset_path=task_file)
    ...
    def _extract_relative_landmarks(self, image):
        ...
        # FIX M1-05b: Tasks API (IMAGE mode)
        ...
        # Lấy 468 landmark đầu (bỏ 10 điểm iris) để khớp input_size 936
        landmark_array = np.array(
            [[lm.x, lm.y] for lm in results.face_landmarks[0][:468]])
...
def prepare_dataset():
        ...
        for txt_file in tqdm(os.listdir(nonstroke_path)):
            ...
            try:
                with open(txt_path, 'r') as f:
                    ...
                    parts = line.split()
                    # FIX M1-05: nhãn lấy theo THƯ MỤC, KHÔNG lấy token đầu
                    # .txt (token trong dataset này ĐẢO NGƯỢC tên thư mục:
                    # NonStroke=1, Stroke=0 — kiểm chứng 07/09)
                    label = 0
        ...
        for txt_file in tqdm(os.listdir(stroke_path)):
            ...
            try:
                with open(txt_path, 'r') as f:
                    line = f.readline().strip()
                    parts = line.split()
                    # FIX M1-05: nhãn theo thư mục (xem chú thích NonStroke)
                    label = 1
    ...
    # Split dataset — BLOCK-AWARE (fix L-04: chống leakage near-duplicate)
    # Các ảnh img_NNNN là frame liên tiếp trích từ video; nếu chia ảnh ngẫu
    # nhiên thì frame gần-trùng nhau nằm cả 2 tập → accuracy phình to.
    # Giải pháp: gom frame theo dải số (mỗi 50 số = 1 block) và chia THEO BLOCK.
    df['block_id'] = (
        df['label'].astype(str) + '_' +
        (df['image_path'].str.extract(r'img_(\d+)')[0]
         .astype(float).fillna(-1).astype(int) // 50).astype(str)
    )
    ...
    val_blocks, test_blocks = train_test_split(
        ...
        random_state=CONFIG["random_seed"],
        stratify=temp_blocks['label']
    )
    train_df = df[df['block_id'].isin(train_blocks['block_id'])]
    val_df = df[df['block_id'].isin(val_blocks['block_id'])]
    test_df = df[df['block_id'].isin(test_blocks['block_id'])]

    print(f"\nDataset split (BLOCK-AWARE — {df['block_id'].nunique()} blocks):")
    print(f"  Train: {len(train_df)} ảnh / {len(train_blocks)} blocks")
    print(f"  Val:   {len(val_df)} ảnh / {len(val_blocks)} blocks")
    print(f"  Test:  {len(test_df)} ảnh / {len(test_blocks)} blocks")
    print(f"  Test stroke/nonstroke: "
          f"{(test_df['label'] == 1).sum()}/{(test_df['label'] == 0).sum()}")
