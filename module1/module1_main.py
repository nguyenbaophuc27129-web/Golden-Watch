...
class Config:
    ...
    @property
    def ML_MODEL(self):
        return os.path.join(self.parent_dir, "models/stroke_classifier_100percent_best.pth")

    @property
    def SCALER(self):
        return os.path.join(self.parent_dir, "models/stroke_classifier_100percent_scaler.pkl")
...
class Module1StrokeDetector:
    ...
    def __init__(self, config=None):
        ...
        print("[Module 1] Initialized successfully - module1_main.py:146")

    def _init_mediapipe(self):
        """Initialize MediaPipe Face Landmarker"""
        print("[Module 1] Loading MediaPipe... - module1_main.py:150")
        self.detector = FaceAsymmetryDetector(model_path=self.config.MEDIAPIPE_MODEL)
        print("[Module 1] MediaPipe loaded - module1_main.py:152")

    def _init_ml_model(self):
        """Initialize PyTorch ML Model"""
        print("[Module 1] Loading ML model... - module1_main.py:156")
        ...
        if torch_version >= (2, 6):
            ...
            self.ml_model.load_state_dict(
                torch.load(self.config.ML_MODEL, map_location='cpu')
            )
        self.ml_model.eval()
        print("[Module 1] ML model loaded - module1_main.py:170")

        # Load scaler
        print("[Module 1] Loading scaler... - module1_main.py:173")
        self.scaler = joblib.load(self.config.SCALER)
        print("[Module 1] Scaler loaded - module1_main.py:175")
    ...
    def _init_export(self):
        ...
        # Create CSV with headers (updated with stroke metrics)
        with open(self.export_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                'timestamp', 'frame_id', 'status', 'stroke_prob',
                'mouth_ratio', 'eye_ratio', 'face_tilt', 'nasolabial_ratio', 'forehead_ratio',
                'nihss_item_4',
                'yawn_detected', 'drowsy_detected', 'head_turn_detected', 'smile_detected'
            ])

        print(f"[Module 1] Export initialized: {self.export_file} - module1_main.py:195")
    ...
    def process_frame(self, frame):
        ...
        return {
            ...
            'landmarks': landmarks,
            'raw_metrics': result.get('raw_metrics', {}),  # Stroke detection metrics from detector
            'nihss_item_4': result.get('nihss_item_4', 0)     # NIHSS Item 4 score
        }

    def export_result(self, result):
        """Export result to CSV with stroke detection metrics"""
        ...
        with open(self.export_file, 'a', newline='') as f:
            ...
            writer.writerow([
                ...
                # Stroke detection metrics (5 biomedical indicators)
                f"{result.get('raw_metrics', {}).get('mouth_ratio', 0):.4f}",  # Mouth asymmetry %
                f"{result.get('raw_metrics', {}).get('eye_ratio', 0):.4f}",    # Eye deviation %
                f"{result.get('raw_metrics', {}).get('face_tilt', 0):.2f}",    # Face tilt degrees
                f"{result.get('raw_metrics', {}).get('nasolabial_ratio', 0):.4f}",  # Nasolabial fold %
                f"{result.get('raw_metrics', {}).get('forehead_ratio', 0):.4f}",  # Forehead asymmetry %
                # NIHSS Item 4 score
                result.get('nihss_item_4', 0),
                # False positive indicators (optional, for reference)
    ...
    def run_camera(self):
        ...
        print(f"[Module 1] Starting camera... - module1_main.py:576")
        print("[Module 1] Press 'q' to quit, 's' to save screenshot, 'f' to toggle fullscreen - module1_main.py:577")

        cap = cv2.VideoCapture(0)

        if not cap.isOpened():
            print("[Module 1] ERROR: Cannot open camera - module1_main.py:582")
            return
        ...
        while self.running:
            ...
            if key == ord('q'):
                ...
                print(f"[Module 1] Screenshot saved: {filename} - module1_main.py:628")
            elif key == ord('f'):
                fullscreen = not fullscreen
                print(f"[Module 1] Fullscreen: {fullscreen} - module1_main.py:631")

        cap.release()
        cv2.destroyAllWindows()

        print(f"[Module 1] Results exported to: {self.export_file} - module1_main.py:636")
        print("[Module 1] Stopped - module1_main.py:637")
...
def main():
    """Main entry point"""
    print("= - module1_main.py:646"*60)
    print("MODULE 1: FACIAL ASYMMETRY DETECTION (PSCS v8.0) - module1_main.py:647")
    print("= - module1_main.py:648"*60)
    print()
    ...
    print()
    print("[Module 1] Session ended - module1_main.py:658")
